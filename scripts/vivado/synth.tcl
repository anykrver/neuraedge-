...
# Override generics for demo config (8x8 DVS sensor, 2x2 mesh).
# For N-MNIST (34x34 DVS sensor), change SENSOR_W=34 SENSOR_H=34
# and set NEURON_ADDR_W=8 (TILE=17x17, max_nid=577 > 2^8=256 — also
# widen NUM_NEURONS to 256 and rebuild weights with train_nmnist.py).
# Mismatch between training sensor size and RTL parameter triggers a
# $fatal assertion in event_encoder at elaboration time.
set_property generic {
    NUM_COLS=2 NUM_ROWS=2 NUM_NEURONS=64 NUM_SYNAPSES=128
    WEIGHT_W=8 MEM_WIDTH=8 THRESHOLD=200 LEAK_SHIFT=1
    TRACE_W=6 A_PLUS=4 A_MINUS=2 TRACE_INCR=16 TRACE_DECAY=3
