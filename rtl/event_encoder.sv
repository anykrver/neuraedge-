...
// Description: DVS event camera → spike packet encoder.
//
// Change log:
//   v1.0 — initial release
//   v1.1 — BUG-7 fix (tile param); BUG-8 fix (neuron_id width)
//   v1.2 — PACKET_W promoted to parameter for port-list compliance
//   v2.0 — Converted to SystemVerilog (.sv):
//            reg → logic; always @(posedge clk) → always_ff;
//            integer loop vars → int.
//            No functional changes.
//
// Tile constraint (enforced by $fatal at elaboration):
//   TILE_W * TILE_H * 2 <= 2^NEURON_ADDR_W
//   Default (8×8 sensor, 2×2 mesh): 4*4*2=32 <= 64=2^6  OK
//
