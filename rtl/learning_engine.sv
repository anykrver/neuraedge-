// learning_engine.sv
// Trace-based STDP weight update engine for NeuraEdge.
// Spike events are queued and scanned across all synapses (LTP/LTD).
// 3-stage pipeline: ST_SCAN_RD → ST_SCAN_WR → ST_SCAN_COMMIT.
...
module learning_engine #(
    ...
    parameter int TRACE_W       = 6,   // reduced from 8 — halves trace toggle density, negligible learning quality impact
    ...
    input  logic                    clk_en,        // clock enable — gate all FFs when idle
    ...
    always_ff @(posedge clk) begin
        if (!rst_n_pipe) begin
            ...
            // Traces: compute next-state combinationally then register once.
            // clk_en=0 when idle (no spikes, empty queue) — all FFs hold, no toggle power.
            // Single NBA per register eliminates the dual-NBA fragility.
            for (int n = 0; n < NUM_NEURONS; n++) begin
                logic [TRACE_W-1:0] pre_leaked, post_leaked;
                pre_leaked  = pre_trace [n] - (pre_trace [n] >> TRACE_DECAY);
                post_leaked = post_trace[n] - (post_trace[n] >> TRACE_DECAY);

                if (spikes_valid && pre_spike[n])
                    pre_trace[n] <= (pre_leaked > ({TRACE_W{1'b1}} - TRACE_W'(TRACE_INCR)))
                                    ? {TRACE_W{1'b1}}
                                    : pre_leaked + TRACE_W'(TRACE_INCR);
                else
                    pre_trace[n] <= pre_leaked;

                if (spikes_valid && post_spike[n])
                    post_trace[n] <= (post_leaked > ({TRACE_W{1'b1}} - TRACE_W'(TRACE_INCR)))
                                     ? {TRACE_W{1'b1}}
                                     : post_leaked + TRACE_W'(TRACE_INCR);
                else
                    post_trace[n] <= post_leaked;
            end
